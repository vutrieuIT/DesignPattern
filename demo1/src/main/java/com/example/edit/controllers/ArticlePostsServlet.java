...
public class ArticlePostsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (path) {
            case "/Index":
                List<Articles> list = articleBridge.findDraftArticles();
            ...
            case "/Search":
              try {
                  ...
                  int count  = articleBridge.getTotalArtilceBySearh(text);
                  int endPage = count/6;
                  if(count  % 6!=0 ) {
                      endPage++;
                  }
                  ...
                  {
                      List<Articles> listA = articleBridge.findSearchPagging(text, index);
                      ...
                      {
                          List<Articles> listA = articleBridge.findSearchPaggingToPre(text,index);
                          request.setAttribute("listA", listA);
                      }
                      else {
                          List<Articles> listA = articleBridge.findSearchPagging(text, index);
                  ...
                  List<Tag> listT = tagBridge.findByindex();
